...
const state = {
  ...
  setRecords: {
    ...(window.USE_SAMPLE_DATA ? window.sampleSetRecords : {}),
    ...JSON.parse(localStorage.getItem('liftflow-set-records') || '{}')
  },
  restRemaining: 120,
  restTimer: null,
  rootMode: 'record'
...
function renderSetRecords() {
  ...
  if (!records.length) { el.setRecords.innerHTML = '<div class="sub">No records yet.</div>'; return; }
...
async function renderTraining() {
  ...
  el.lastPerformed.textContent = `Previous: ${ex.lastDate}`;
...
el.openSetupBtn.addEventListener('click', openSetupModal);
el.closeSetupBtn.addEventListener('click', closeSetupModal);
el.driveSyncBtn.addEventListener('click', async () => {
  try {
    await window.driveApi.auth();
    const [profileRes, recordsRes] = await Promise.allSettled([window.driveApi.loadProfile(), window.driveApi.loadRecords()]);
    if (profileRes.status === 'fulfilled' && profileRes.value?.profile) {
      saveProfileLocal(profileRes.value.profile);
    }
    if (recordsRes.status === 'fulfilled' && recordsRes.value?.records) {
      state.setRecords = recordsRes.value.records;
      saveLocal();
      renderSetRecords();
      renderTrainingTrend();
      renderHistorySummary();
