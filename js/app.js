...
const state = {
  ...
  selectedHistoryDate: '2026-03-31',
...
function getRecordsForDate(date) {
  return Object.keys(state.setRecords).filter(key => key.startsWith(`${date}__`)).flatMap(key => {
...
function getTotalVolumeForDate(date, filter = 'total') {
  return getRecordsForDate(date).filter(r => matchFilter(r.exercise, filter)).reduce((sum,r) => sum + r.reps * r.weight, 0);
}
function getAvgWeightForDate(date) {
  const records = getRecordsForDate(date);
  if (!records.length) return 0;
  return Math.round((records.reduce((sum,r)=>sum+r.weight,0) / records.length) * 10) / 10;
}
function getTotalRepsForDate(date) { return getRecordsForDate(date).reduce((sum,r)=>sum+r.reps,0); }
function getSetCountForDate(date) { return getRecordsForDate(date).length; }
function getAllDatesWithTraining() { return [...new Set(Object.keys(state.setRecords).map(k => k.split('__')[0]))].sort(); }
...
function makeSuggestion(exerciseName) {
  ...
  const text = days < 2 ? 'Previous session is close, so today prioritizes quality and repeatability over pushing load.' : 'Recent trend is stable, so today aims to keep quality high and slightly raise total work.';
...
function makeRecoverySuggestion() {
  ...
  return { protein, water, sleep, carb, text: todayVol > 0 ? 'Based on today’s logged work, prioritize carbs, hydration, and protein for recovery.' : 'Base recovery target for a lighter or non-training day.' };
}
async function refreshAiSuggestions() {
  const payload = { profile: state.profile, selectedExercise: state.selectedExercise, selectedDate: state.selectedDate, records: state.setRecords, variationSeed: Date.now() };
  try {
...
function renderExerciseOptions() {
  ...
  Object.entries(window.EXERCISES).filter(([, v]) => v.group === state.selectedGroup).forEach(([name]) => {
...
function renderBodyMap() {
  document.querySelectorAll('#frontBody [id], #backBody [id]').forEach(node => { if (node.tagName !== 'svg') node.setAttribute('fill', '#1f2937'); });
...
function renderSetRecords() {
  ...
  if (!records.length) { el.setRecords.innerHTML = '<div class="sub">まだ記録がありません。</div>'; return; }
  el.setRecords.innerHTML = records.map((r,i) => `<div class="record-item"><div><div class="record-main">${r.set} set / ${r.reps} reps / ${r.weight}kg</div><div class="record-sub">Volume ${r.reps * r.weight}</div></div><button data-remove="${i}" style="width:auto;">Delete</button></div>`).join('');
...
function renderTrainingTrend() {
  ...
  const left = dates.map(d => ({ label: `${Number(d.slice(5,7))}/${Number(d.slice(8,10))}`, value: getTotalVolumeForDate(d) }));
  const right = dates.map(d => ({ label: `${Number(d.slice(5,7))}/${Number(d.slice(8,10))}`, value: getAvgWeightForDate(d) }));
...
function aggregateSeries(range) {
  ...
  if (range === 'daily') {
    return {
      left: dates.map(d => ({ label: `${Number(d.slice(5,7))}/${Number(d.slice(8,10))}`, value: getTotalVolumeForDate(d) })),
      right: dates.map(d => ({ label: `${Number(d.slice(5,7))}/${Number(d.slice(8,10))}`, value: getAvgWeightForDate(d) }))
    };
  }
  if (range === 'weekly') {
    const weeks = {};
    dates.forEach(d => {
      ...
      const avg = getAvgWeightForDate(d); if (avg) weeks[key].avgList.push(avg);
    });
    const keys = Object.keys(weeks);
    return {
      left: keys.map(k => ({ label: k, value: weeks[k].volume })),
      right: keys.map(k => ({ label: k, value: Math.round((weeks[k].avgList.reduce((a,b)=>a+b,0) / Math.max(weeks[k].avgList.length,1)) * 10) / 10 }))
    };
  }
  const months = {};
  dates.forEach(d => {
    ...
    const avg = getAvgWeightForDate(d); if (avg) months[key].avgList.push(avg);
  });
  const keys = Object.keys(months);
  return {
    left: keys.map(k => ({ label: k, value: months[k].volume })),
    right: keys.map(k => ({ label: k, value: Math.round((months[k].avgList.reduce((a,b)=>a+b,0) / Math.max(months[k].avgList.length,1)) * 10) / 10 }))
  };
}
function renderCalendar() {
  ...
  ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].forEach(d => { const node = document.createElement('div'); node.className = 'dow'; node.textContent = d; el.calendarGrid.appendChild(node); });
  for (let i = 0; i < firstDay; i++) { const blank = document.createElement('div'); blank.className = 'day empty'; el.calendarGrid.appendChild(blank); }
  for (let d = 1; d <= lastDate; d++) {
    ...
    btn.addEventListener('click', () => { state.selectedHistoryDate = dateStr; renderHistorySummary(); renderCalendar(); });
    el.calendarGrid.appendChild(btn);
  }
}
function renderHistorySummary() {
  const date = state.selectedHistoryDate;
  el.summaryVolume.textContent = getTotalVolumeForDate(date) || '-';
  el.summaryAvgWeight.textContent = getAvgWeightForDate(date) || '-';
  el.summaryReps.textContent = getTotalRepsForDate(date) || '-';
  el.summarySets.textContent = getSetCountForDate(date) || '-';
...
function syncRestFromInput() { state.restRemaining = Math.max(0, Number(el.restMin.value||0)*60 + Number(el.restSec.value||0)); renderRest(); }
function renderRest() { const m=Math.floor(state.restRemaining/60), s=state.restRemaining%60; el.restDisplay.textContent = `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`; }
function notifyRestEnd() {
  try {
    const AudioCtx = window.AudioContext || window.webkitAudioContext;
    if (AudioCtx) {
      const ctx = new AudioCtx(), osc = ctx.createOscillator(), gain = ctx.createGain();
      osc.type='sine'; osc.frequency.value=720; osc.connect(gain); gain.connect(ctx.destination);
...
function startRestTimer() {
  ...
  state.restTimer = setInterval(() => {
    ...
    renderRest();
    if (state.restRemaining <= 0) { clearInterval(state.restTimer); state.restTimer = null; notifyRestEnd(); }
  }, 1000);
}
async function saveRecordsEveryTime() {
  try { await window.driveApi.saveRecords(state.setRecords); } catch (e) { console.error(e); }
}
async function addSetRecord() {
  const key = getSetKey(state.selectedDate, state.selectedExercise);
  const arr = state.setRecords[key] || [];
  arr.push({ set:Number(el.setNumber.value), reps:Number(el.repNumber.value), weight:Number(el.weightNumber.value) });
...
function initSelectors() {
  ...
  for (let i=1;i<=8;i++) el.setNumber.insertAdjacentHTML('beforeend', `<option value="${i}">${i} set</option>`);
  for (let i=1;i<=20;i++) el.repNumber.insertAdjacentHTML('beforeend', `<option value="${i}">${i} reps</option>`);
  for (let i=0;i<=220;i+=2.5) el.weightNumber.insertAdjacentHTML('beforeend', `<option value="${i}">${i}kg</option>`);
  el.muscleGroupSelect.value = state.selectedGroup;
  el.exerciseSelect.value = state.selectedExercise;
}

document.querySelectorAll('.tab').forEach(tab => tab.addEventListener('click', () => {
  state.rootMode = tab.dataset.tab; renderRootMode();
  if (state.rootMode === 'history') { renderCalendar(); renderHistorySummary(); }
}));
...
el.calendarMetricSelect.addEventListener('change', e => { state.selectedCalendarMetric = e.target.value; renderCalendar(); });
...
el.exerciseSelect.addEventListener('change', async e => { state.selectedExercise = e.target.value; await renderTraining(); flashGold(el.suggestionCard); });
...
el.restReset.addEventListener('click', () => { clearInterval(state.restTimer); state.restTimer = null; syncRestFromInput(); flashGold(el.timerCard); });
el.addSetBtn.addEventListener('click', addSetRecord);
el.settingsBtn.addEventListener('click', (e) => { e.stopPropagation(); toggleSettingsMenu(); });
document.addEventListener('click', (e) => {
  if (!el.settingsMenu.contains(e.target) && e.target !== el.settingsBtn) closeSettingsMenu();
});
el.openSetupBtn.addEventListener('click', () => { closeSettingsMenu(); openSetupModal(); });
...
el.saveProfileBtn.addEventListener('click', async () => {
  ...
  try { await window.driveApi.saveProfile(profile); } catch (e) { console.error(e); }
  closeSetupModal();
  await refreshAiSuggestions();
});
...
renderCalendar();
renderHistorySummary();
if (!localStorage.getItem('liftflow-profile')) openSetupModal();
