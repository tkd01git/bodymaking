...
window.ROUTINE_DATA = [
  {
    ...
    action: [
      '量は落とす。質は残す。',
      '新種目・強い筋肉痛が出る刺激は入れない。',
      '下半身高ボリューム、強いエキセントリック、強いプライオは避ける。'
    ],
  ...
  },
  {
    ...
    title: '糖質＋水分',
    action: [
      '糖質1〜2g/kg。80kgなら80〜160g。',
      '脂質・食物繊維は少なめ。慣れた食事にする。',
      '水分5〜10mL/kg。80kgなら400〜800mL。'
    ],
  ...
  },
  {
    ...
    action: [
      '初回本番で高用量にしない。',
      '練習で問題なければ3mg/kgが第一候補。80kgなら240mg。',
      '普段少ないなら100〜200mgから試す。'
    ],
  ...
  },
  {
    ...
    action: [
      '座りっぱなしにしない。',
      '可能なら集合前に2分程度の再ウォームアップ。',
      '軽い弾み、ドリル、10m加速感を1〜2回。'
    ],
  ...
  },
  {
    ...
    action: [
      '次レースがあるなら糖質と水分をすぐ戻す。',
      '糖質1.0〜1.2g/kg/時。80kgなら80〜96g/時。',
      '食べにくいなら糖質0.8g/kg/時＋たんぱく質0.4g/kg/時。'
    ],
  ...
  },
  {
    ...
    title: '太もも冷却は基本なし',
    action: [
      '次レースまで数時間ならルーティンアイシングはしない。',
      '暑熱対策は日陰、送風、水かけ、冷却ベストを優先。',
      '強い痛み・損傷疑いがある場合だけ医療判断を優先。'
    ],
  ...
  },
  {
    ...
    title: '交代浴は翌日向けに条件付き',
    action: [
      '同日レース前には使わない。筋出力が鈍る可能性がある。',
      '翌日へ向けるなら、温浴2〜3分＋冷浴30〜60秒を3〜5セット。合計10〜15分程度。',
      '目的は主観的疲労・筋痛の軽減。筋出力を即時に上げる目的では使わない。'
    ],
    claim: '交代浴は受動回復より筋痛や筋力低下を軽減する可能性があるが、研究の質には限界があり、短いインターバルでの筋出力回復には不確実性が残る。',
    method: 'CWTの系統的レビュー・メタ解析は18試験を対象に、運動誘発性筋損傷後の筋痛・筋力低下を受動回復などと比較した。日本の理学療法領域の報告では、交代浴で血中乳酸は低下したが筋出力改善は明確でなかった。',
    result: 'メタ解析ではCWTが受動回復より筋痛と筋力低下を改善したが、試験は高バイアスリスク。日本の報告では短時間インターバルの筋力回復処置としては不向きの可能性が示された。',
    source: 'Bieuzen et al., 2013, PLOS ONE; 内堀ほか, 2006, 日本理学療法学術大会; 松尾ほか, 2008, 日本理学療法学術大会.'
  }
];
