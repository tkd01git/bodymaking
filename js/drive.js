window.driveApi = {
  ...
  async saveSleeping(sleeping) {
    const res = await fetch(window.DRIVE_ENDPOINTS.saveSleeping, {
      method: 'POST',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sleeping })
    });
    if (!res.ok) throw new Error('Drive sleeping save failed');
    return res.json();
  },

  async loadSleeping() {
    const res = await fetch(window.DRIVE_ENDPOINTS.loadSleeping, { credentials: 'include' });
    if (!res.ok) throw new Error('Drive sleeping load failed');
    return res.json();
  }
};
